...
When no honest loop can be built, stop and request the missing artifact or environment. Do not guess a fix.
...
Test one variable at a time. Prefer debugger or inspection and targeted boundary instrumentation over broad logging.

Temporary instrumentation must have a unique marker and be removed before completion.

## 4. Fix through the correct seam

Turn the minimal reproduction into a failing regression test at the highest seam that genuinely reaches the bug. When no suitable seam exists, record the architectural deficiency rather than adding a misleading shallow test.

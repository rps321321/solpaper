...
If the question can be answered reliably from primary documentation alone, use `solpaper-research` instead.

## Choose the prototype shape

- **State/domain uncertainty** → small Rust harness that drives scenarios through a public model interface.
- **Win32/COM feasibility** → isolated spike crate under `spikes/` with minimal dependencies and explicit unsafe boundaries.
- **Interaction/visual uncertainty** → multiple clearly different variations or one focused interaction harness; do not polish a single assumed answer.
- **Performance uncertainty** → reproducible benchmark/harness with named build mode, hardware, workload, and baseline.
...
- screenshots/recordings only when they add information and contain no private data;
- unresolved risks;
